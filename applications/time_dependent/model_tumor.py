...
def main(args):
    ...
    f = dl.Constant(0.0)
    lmbda = dl.Constant(1.0)
    bc = []
    bc0 = []
    u0_expr = dl.Expression("std::exp( - 100.*(x[0]-.5)*(x[0]-.5) - 100.*(x[1]-.75)*(x[1]-.75) )", element = Vh[STATE].ufl_element() )
    u0 = dl.interpolate(u0_expr, Vh[STATE])
    
    # Set up time stepping and variational problem.
    dt = (T_FINAL - T_INIT)/args.nt
    pde_varf = TumorEquationVarf(dt, f, lmbda)
    ...
    with dl.XDMFFile(mesh.mpi_comm(), "results/tumor/pointwise_variance.xdmf") as fid:
    ...
    with dl.XDMFFile(mesh.mpi_comm(), "results/tumor/results.xdmf") as fid:
        fid.parameters["functions_share_mesh"] = True
        fid.parameters["rewrite_function_mesh"] = False
    ...
    pde.exportState(x[STATE], "results/tumor/state.xdmf")
    ...
    with dl.XDMFFile(mesh.mpi_comm(), "results/tumor/samples.xdmf") as fid:
        fid.parameters["functions_share_mesh"] = True
        fid.parameters["rewrite_function_mesh"] = False
    ...
    U.export(Vh[PARAMETER], "results/tumor/evec.xdmf", varname="gen_evec", normalize=True)
    if rank == 0:
        np.savetxt("results/tumor/eigenvalues.txt", d)
